...
def _to_shop_order_out(db: Session, so: models.ShopOrder) -> schemas.ShopOrderOut:
    ...
    return schemas.ShopOrderOut(
        ...
        shop_address=shop.address, shop_phone=shop.owner.phone, shop_latitude=shop.latitude, shop_longitude=shop.longitude,p_phone=shop.owner.phone, shop_latitude=shop.latitude, shop_longitude=shop.longitude,p_latitude=shop.latitude, shop_longitude=shop.longitude,
...
def create_master_item(payload: ItemCreatePayload,
                        db: Session = Depends(get_db),
                        user: models.User = Depends(require_shopkeeper)):
    """Agar catalog mein item pehle se nahi hai (jaise 'dal', 'chini'), to yeh banata hai."""
    clean_name = payload.name.strip()
    clean_image = payload.image_url.strip() if payload.image_url and payload.image_url.strip() else None
    ...
    if existing:
        if clean_image and not existing.image_url:
            ...
            db.refresh(existing)
        return existing
    item = models.Item(name=clean_name, unit=payload.unit, category=payload.category, image_url=clean_image)
    db.add(item)
    db.commit()
    db.refresh(item)
    return item
...
def toggle_shop_status(payload: schemas.ShopStatusUpdate, db: Session = Depends(get_db),
    ...
    db.commit()
    return {"status": "ok", "is_open": shop.is_open}
@router.get("/orders/{shop_order_id}/messages", response_model=list[schemas.ChatMessageOut])
def get_messages_shop(
...
@router.post("/orders/{shop_order_id}/messages", response_model=schemas.ChatMessageOut)
async def send_message_shop(
    shop_order_id: int, payload: schemas.ChatMessageCreate,
    db: Session = Depends(get_db), user: models.User = Depends(require_shopkeeper),
):
    shop = _get_owned_shop(db, user)
    so = db.query(models.ShopOrder).get(shop_order_id)
    if not so or so.shop_id != shop.id:
    ...
    msg = models.ChatMessage(
        ...
        sender_role=models.UserRole.shopkeeper, message=text,
    )
    db.add(msg)
    db.commit()
    db.refresh(msg)
    ...
    await realtime.manager.send_to_user(
        so.order.customer_id, "new_message",
        {"shop_order_id": shop_order_id, "sender_name": shop.name, "message": text},
    )
    return schemas.ChatMessageOut(
        id=msg.id, sender_id=msg.sender_id, sender_role=msg.sender_role,
        sender_name=shop.name, message=msg.message, created_at=msg.created_at,
    )
